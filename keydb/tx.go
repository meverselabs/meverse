...
func (dbi *dbItem) writeSetTo(buf []byte) []byte {
	...
	binutil.LittleEndian.PutUint32(buf[len(buf)-4:], uint32(len(dbi.key)))
	buf = append(buf, dbi.key...)

	buf = append(buf, []byte{0, 0, 0, 0}...)
	binutil.LittleEndian.PutUint32(buf[len(buf)-4:], uint32(len(dbi.data)))
...
func (tx *Tx) Set(key []byte, value interface{}) error {
	if tx.db == nil {
		return ErrTxClosed
	} else if !tx.writable {
		return ErrTxNotWritable
