...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	addrCopy := addr
	isExtContract := evm.StateDB.IsExtContract(addrCopy)

	if !isExtContract && !evm.StateDB.Exist(addr) {
	...
	if isPrecompile {
		...
		ret, err = evm.StateDB.Exec(caller.Address(), addrCopy, input)
		gas = 0
		//log.Println("ret in evm.Call", ret)
	} else {
...
func (evm *EVM) DelegateCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	if p, isPrecompile := evm.precompile(addr); isPrecompile {
		ret, gas, err = RunPrecompiledContract(p, input, gas)
	} else if evm.StateDB.IsExtContract(addr) {
