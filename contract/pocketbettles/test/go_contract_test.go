...
func TestConverter(t *testing.T) {
	...
	inf, err = tc.MakeTx(util.AdminKey, converterAddr, "converter", amount.MustParseAmount("12"))
	log.Println(inf, err)
	...
	inf, err = tc.MakeTx(util.AdminKey, converterAddr, "call", amount.MustParseAmount("11"))
...
func mintGenesis(tc *util.TestContext) (genesisAddr common.Address, err error) {
	...
	_, err = tc.MakeTx(util.AdminKey, genesisAddr, "Name")
	return
...
func TestMintingRound1(t *testing.T) {
	...
	inf, err := tc.MakeTx(util.AdminKey, minterAddr, "roundSetting", "20", "99", "100", "199", "200", "299", "100", "2500", "2150")
	if err != nil {
		log.Println(inf, err)
		t.Error(err)
		return
	...
	inf, err = tc.MakeTx(util.AdminKey, minterAddr, "mint", "3")
	if err == nil {
	...
	inf, err = tc.MakeTx(util.AdminKey, tc.MainToken, "Approve", minterAddr, "100000000000000")
	if err != nil {
	...
	tc.MakeTx(util.AdminKey, tc.MainToken, "Approve", formulatorAddr, amount.MustParseAmount("1000000000000000000"))

	inf, _ = tc.MakeTx(util.AdminKey, formulatorAddr, "CreateAlphaBatch", 5)
	addrs := inf[0].([]common.Address)
	inf, err = tc.MakeTx(util.AdminKey, formulatorAddr, "CreateSigma", addrs[:4])
	if err == nil {
	...
	_, err = tc.MakeTx(util.AdminKey, formulatorAddr, "CreateSigma", addrs[:4])
	if err != nil {
	...
	inf, err = tc.MakeTx(util.AdminKey, nftAddr, "setMinter", minterAddr)
	if err != nil {
		t.Errorf("not expect must mint nft %v, %v", inf, err)
		return
	}

	inf, err = tc.MakeTx(util.AdminKey, minterAddr, "mint", "4")
	if err == nil {
	...
	inf, err = tc.MakeTx(util.AdminKey, minterAddr, "mint", "0")
	if err == nil {
		t.Errorf("expect err You can only mint up to 3 heroes in one transaction %v, %v", inf, err)
		return
	}

	inf, err = tc.MakeTx(util.AdminKey, minterAddr, "mint", "1")
	if err != nil {
		t.Errorf("not expect err %v, %v", inf, err)
		return
	}
	...
	_, err = tc.MakeTx(util.AdminKey, formulatorAddr, "Revoke", addrs[0])
	if err != nil {
...
func TestMintingRound2(t *testing.T) {
	...
	_, err = tc.MakeTx(util.AdminKey, formulatorAddr, "Revoke", addrs[4])
	if err != nil {
		t.Errorf("invalid Revoke sigma")
		return
	}
	...
	inf, err = tc.MakeTx(util.AdminKey, genesisAddr, "Mint", "1")
	if err != nil {
		t.Errorf("expect err %v, %v", inf, err)
		return
	}
	...
	inf, err = tc.MakeTx(util.AdminKey, genesisAddr, "Burn", gaddr)
	if err != nil {
		t.Errorf("expect err %v, %v", inf, err)
		return
	}
	...
	inf, err = tc.MakeTx(util.AdminKey, minterAddr, "addCommunityEventAddr", []common.Address{util.Admin})
	if err != nil {
		t.Errorf("expect err Alpha, Sigma, Omega Formulator holders %v, %v", inf, err)
		return
	}
...
func TestMintover(t *testing.T) {
	...
	inf, err := tc.MakeTx(util.AdminKey, minterAddr, "roundSetting", "1", "2", "3", "4", "5", "400", "1", "1", "100")
	if err != nil {
		log.Println(inf, err)
		t.Error(err)
		return
...
func TestMintingFailBetweenRound(t *testing.T) {
	...
	inf, err = tc.MakeTx(util.AdminKey, minterAddr, "roundSetting", "40", "60", "80", "100", "120", "140", "100", "2500", "2150")
	if err != nil {
		log.Println(inf, err)
		t.Error(err)
		return
