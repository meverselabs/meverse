...
func TestExecuteContractTx(t *testing.T) {
	...
	inf, err := tc.SendTx(util.AdminKey, farmAddr, "Withdraw", uint64(0), amount.MustParseAmount("10000000000"))
	log.Println(inf, err)
	printTotalSupply(tc, tokenAddr)
	...
	inf, err = tc.SendTx(util.AdminKey, farmAddr, "Deposit", uint64(0), amount.MustParseAmount("5423.34248643"))
	log.Println(inf, err)
	printTotalSupply(tc, tokenAddr)
