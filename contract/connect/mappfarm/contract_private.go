...
func (cont *FarmContract) _poolInfo(cc *types.ContractContext) (*PoolInfo, error) {
	bs := cc.ContractData([]byte{tagPoolInfo})
...
func (cont *FarmContract) _wantLockedTotal(cc *types.ContractContext) *amount.Amount {
	return cont.pool.WantLockedTotal(cc)
}

func (cont *FarmContract) setPoolInfo(cc *types.ContractContext, pool *PoolInfo) error {
	bf := new(bytes.Buffer)
	_, err := pool.WriteTo(bf)
	if err != nil {
		return err
	}
	cc.SetContractData([]byte{tagPoolInfo}, bf.Bytes())
	return nil
}
