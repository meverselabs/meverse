...
func TestExecuteContractTx(t *testing.T) {
	...
	poolContArgs := &mapppool.PoolContractConstruction{
		Farm:              farmAddr,
		Want:              tokenAddr,
	...
	if initPoolRes, err := tc.MakeTx(util.AdminKey, farmAddr, "InitPool", false, bf.Bytes()); err != nil {
		log.Println(initPoolRes)
		panic(err)
	}
