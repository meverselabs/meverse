...
func tokenURI(cc *types.ContractContext, _tokenId hash.Hash256) string {
...
func mint(cc *types.ContractContext, contaddr common.Address, count *big.Int) ([]hash.Hash256, error) {
	if !isOwner(cc) {
	...
	hs := []hash.Hash256{}
	for nc.Cmp(limit) < 0 {
		nftID, err := _mintNFTWithIndex(cc, contaddr, nc)
		if err != nil {
...
func MintBatch(cc *types.ContractContext, contaddr common.Address, addrs []common.Address) ([]hash.Hash256, error) {
	if !isOwner(cc) {
		return nil, errors.New("doesn't have mint permission")
	}
...
func setTokenURI(cc *types.ContractContext, _tokenId hash.Hash256, uri string) error {
	if !isOwner(cc) {
		return errors.New("doesn't have mint permission")
	}
...
func safeTransferFrom(cc *types.ContractContext, contaddr common.Address, _from common.Address, _to common.Address, _tokenId hash.Hash256, data []byte) error {
	if err := transferFrom(cc, _from, _to, _tokenId); err != nil {
		return err
	}

	if in, err := cc.Exec(cc, _to, "OnERC721Received", []interface{}{contaddr, _from, _tokenId, data}); err != nil {
		return err
	} else if len(in) == 0 {
