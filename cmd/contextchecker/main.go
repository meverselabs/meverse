...
type Config struct {
	...
	CheckInitGenesisHash string
	CheckInitHash        string
	CheckInitHeight      uint32
	CheckInitTimestamp   uint64

	RunMode string // Test, Sync

	CheckStoreRoot string
	ZipHeight      uint32
	ZipInterval    uint32
	SaveZipPath    string
}
...
func main() {
	...
	case "Sync":
		...
		cm.Wait()
	case "Test":
		testContext(st, cfg, "./checker_config.toml")
	}
}
